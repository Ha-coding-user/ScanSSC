...
class ScanSSC(BaseModule):
    ...
    def extract_img_feat(self, img_inputs, img_metas):
        ...
        if depth is not None:
            ...
            mlvl_dpt_dists = None
        depth_feats, height_feats, width_feats = self.VoxFormer_head(
                                                                    [context],
                                                                    proposal,
        ...
        return depth_feats, height_feats, width_feats, depth
    
    def occ_encoder(self, depth_feats, height_feats, width_feats):
        if self.occ_encoder_backbone is not None:
            x = self.occ_encoder_backbone(depth_feats, height_feats, width_feats)
    ...
    def forward_train(self, data_dict):
        ...
        depth_feats, height_feats, width_feats, depth = self.extract_img_feat(img_inputs, img_metas)
        vox_feats = self.occ_encoder(depth_feats, height_feats, width_feats)
        
        if type(vox_feats) is not list:
            vox_feats = [vox_feats]
